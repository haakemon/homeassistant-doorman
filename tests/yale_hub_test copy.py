...
from custom_components.doorman.yale.hub import YaleHub
...
@ddt
class YaleHubMethods(unittest.TestCase):
    ...
    def test_login_access_token_present(self, _username, _password, _zone_id, expected_result):
        YaleHubMethods._LOGGER.info("Test: test_login_access_token_present")
        yale_hub = YaleHub(_username, _password, _zone_id)

        login_data = yale_hub.login()

        actual_result = "access_token" in login_data
